module.exports = {
  name: "If dashboard interacted",

  displayName: "If dashboard interacted",
  ...
  async action(cache) {
    ...
    // Check if interaction has changed within the last 30 seconds
    const currentTime = new Date().getTime();
    const interactionTime = interaction.timestamp || 0; // Assuming timestamp is available
    const timeDifference = currentTime - interactionTime;

    if (timeDifference > 30000) { // 30 seconds
      this.storeValue(false, storage, varName2, cache); // Interaction hasn't changed
    } else {
      switch (info) {
        case 0:
